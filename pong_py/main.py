...
def run_neat(config):
    ...
    p.add_reporter(neat.Checkpointer(1, filename_prefix=checkpoint_file))

    winner = p.run(eval_genomes, 50)
    with open(os.path.join(checkpoint_dir, "best.pickle"), "wb") as f:
...
def test_ai(config):
    ...
    with open(os.path.join(checkpoint_dir, "best.pickle"), "rb") as f:
